...
void MyClient::doSendUsersOnline() const
{
    ...
    out << s;
    QVector<QString> messages = _serv->messagesDatabase[_name];
    // QByteArray block;
    // QDataStream out(&block, QIODevice::WriteOnly);
    // out << (quint16)0 << comAllMessages;
    ...
    if (messages.empty()) {
        ...
        // messages.append("me\rto you");
        qDebug() << "init mes len: " << messages.length();
        out << QString::number(messages.length());

        for (int i = 0; i < messages.length(); ++i) {
            QStringList name_message =  messages.at(i).split('\r');
            out << name_message.at(0);
            out << name_message.at(1);
        }
